...
def eval_model(model,train_batch_size=c.BATCH_SIZE * c.TRIPLET_PER_BATCH, test_dir= c.TEST_DIR, check_partial=False, gru_model=None):
    ...
    fprr, tprr, thresholds = metrics.roc_curve(y_true, y_pred)
    auc = metrics.auc(fprr, tprr)
    plt.plot(fprr, tprr, c = 'red', lw = 3, alpha = 0.7, label = 'AUC=%.3f' % auc)
    ...
    plt.title('CNN & GRU', fontsize=17)
    plt.show()
    return fm, tpr, acc, eer, fprr, tprr, auc
...
if __name__ == '__main__':
    ...
    fm, tpr, acc, eer, fprr, tprr, auc = eval_model(model, check_partial=True,gru_model=gru_model)
    print("f-measure = {0}, true positive rate = {1}, accuracy = {2}, equal error rate = {3}, auc = {4}".format(fm, tpr, acc, eer, auc))
